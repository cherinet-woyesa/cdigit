...
const Sidebar: React.FC<SidebarProps> = ({ onNavigate, activeSection, onLogout }) => {
  const navItems = [
    { id: "my-branch", label: "🏦 My Branch", icon: CurrencyDollarIcon, badgeCount: 12 },
    ...
    { id: "Screen-Display", label: "📺 Screen Display", icon: ChartBarIcon },
    { id: "reports", label: "📊 Reports", icon: ChartBarIcon },
    { id: "audit-log", label: "📝 Audit Log", icon: DocumentTextIcon },
    { id: "settings", label: "⚙️ Settings", icon: CogIcon }
  ];
              ...
              return (
                <li key={item.id}>
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-3 text-left rounded-lg transition-all duration-200 group ${isActive
                        ? "bg-white text-fuchsia-700 shadow-lg"
                        : "text-fuchsia-50 hover:bg-fuchsia-600/50 hover:text-white"
                      }`}
                  >
                    <div className="flex items-center">
                      <IconComponent className={`h-5 w-5 mr-3 ${isActive ? "text-fuchsia-700" : "text-fuchsia-100"
                        }`} />
                    ...
                    {item.badgeCount && item.badgeCount > 0 && (
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full min-w-6 text-center ${isActive
