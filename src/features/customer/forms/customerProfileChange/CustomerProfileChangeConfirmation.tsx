...
export default function CustomerProfileChangeConfirmation() {
    ...
    const [changeData, setChangeData] = useState<CustomerProfileChangeData>({});
    ...
    const entityId = getEntityId(changeData);
        ...
        const initializeData = async () => {
            try {
                ...
                if (state?.serverData?.data) {
                    setChangeData(state.serverData.data);
                } else if (state?.updateId) {
                    const response = await getCustomerProfileChangeById(state.updateId);
                    if (response.success && response.data) {
                        setChangeData(response.data);
                    } else {
                        setError(response.message || t('loadFailed', 'Failed to load change details'));
                    }
                } else {
                    setError(t('invalidState', 'Invalid request state. Please start over.'));
                }
            } catch (err: any) {
                setError(err?.message || t('loadFailed', 'Failed to load change details'));
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleNewChange = () => navigate('/form/customer-profile-change');
    const handleBackToDashboard = () => navigate('/dashboard');

    if (isLoading) return <LoadingState message={t('loading', 'Loading change details...')} />;
    ...
    if (error && !entityId) return (
        <ErrorState
            ...
            primaryLabel={t('goToCustomerProfileChange', 'Go to Customer Profile Change Form')}
                ...
                <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                    ...
                    <div ref={componentToPrintRef} className="p-4">
                        <SuccessIcon
                            ...
                            message={t('changeRequestSubmitted', 'Your customer profile change request has been submitted.')}
                        />
                        ...
                        <div className="mb-4">
                            <div className="bg-amber-25 rounded-lg p-4 border border-amber-200 shadow-sm">
                                ...
                                <div className="space-y-2 text-sm">
                                    <SummaryRow icon={Edit} label={t('changeType', 'Change Type')} value={changeData.changeType || 'N/A'} />
                                    <SummaryRow icon={Info} label={t('reason', 'Reason')} value={changeData.reason || 'N/A'} />
                                </div>
                            </div>
                        </div>
