...
export const businessValidation = {
  chequeNumber: (value: string | undefined) => {
    ...
    if (!/^[A-Za-z0-9]+$/.test(value)) return 'Cheque number must contain only letters and numbers';
    return undefined;
  },
  ...
  description: (value: string | undefined, required: boolean = false) => {
    ...
    if (!value?.trim()) return undefined; // Optional field
    if (value.length < 10) return 'Description is too short (minimum 10 characters)';
  ...
  reason: (value: string | undefined) => {
    ...
    if (value.length > 200) return 'Reason is too long (maximum 200 characters)';
    return undefined;
  },
  ...
  urgencyLevel: (value: string | undefined) => {
    ...
    if (!validLevels.includes(value)) return 'Please select a valid urgency level';
    return undefined;
  },
...
export const addressValidation = {
  address: (value: string | undefined) => {
    ...
    if (value.length < 10) return 'Address is too short (minimum 10 characters)';
    if (value.length > 200) return 'Address is too long (maximum 200 characters)';
    return undefined;
  },
...
export const corporateValidation = {
  ...
  businessType: (value: string | undefined) => {
    ...
    const validTypes = ['Sole Proprietorship', 'Partnership', 'Corporation', 'LLC', 'NGO', 'Government'];
    if (!validTypes.includes(value)) return 'Please select a valid business type';
    return undefined;
  }
};
...
export const denominationValidation = {
  ...
  denominationCount: (value: string | number | undefined) => {
    const num = typeof value === 'string' ? parseInt(value) : value;
    if (num === undefined || num === null || isNaN(num)) return 'Count is required';
...
export const checkDepositValidationSchema = {
  ...
  drawerAccNum: accountValidation.accountNumber,
...
export const cashDiscrepancyReportValidationSchema = {
  ...
  description: (value: string | undefined) => businessValidation.description(value, true),
  reason: businessValidation.reason,
  documentReference: (value: string | undefined) => documentValidation.documentReference(value, false)
};
...
export const phoneBlockValidationSchema = {
  ...
  alternativeVerification: (value: string | undefined) => {
    ...
    if (!validMethods.includes(value)) return 'Please select a valid verification method';
    return undefined;
  }
};
...
export const posDeliveryFormValidationSchema = {
  ...addressValidation,
  contactName: personalInfoValidation.fullName,
  contactPhone: personalInfoValidation.phoneNumber,
  deliveryDate: dateValidation.futureDate,
  specialInstructions: (value: string | undefined) => businessValidation.description(value, false)
};
...
export const ticketMandateRequestValidationSchema = {
  ...
  mandateType: (value: string | undefined) => {
    ...
    const validTypes = ['Single Transaction', 'Multiple Transactions', 'Standing Order'];
    if (!validTypes.includes(value)) return 'Please select a valid mandate type';
    return undefined;
  },
